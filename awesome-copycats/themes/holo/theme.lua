...
theme.font                                      = "Roboto Bold 13"
theme.taglist_font                              = "Roboto Condensed Bold 11"
...
lain.widget.calendar({
    attach_to = { calendar_icon, mytextcalendar },
    notification_preset = {
        fg = "#FFFFFF",
        bg = theme.bg_normal,
...
-- Custom mailbox
local mail_icon = wibox.widget.imagebox(theme.mail)
local mbox = lain.widget.mbox({
    settings = function()
        widget:set_markup(space3 .. markup.font(theme.font, ": " .. mbox_now.allnewmail) .. markup.font("Roboto 5", " "))
    end
})
local mboxbg = wibox.container.background(mbox.widget, theme.bg_focus, gears.shape.rectangle)
local mboxwidget = wibox.container.margin(mboxbg, 0, 0, 5, 5)
...
function theme.at_screen_connect(s)
    ...
    systray = wibox.widget.systray(false)
    ...
    s.mybottomwibox = awful.wibar({ position = "bottom", screen = s, border_width = 0, height = 36 })
